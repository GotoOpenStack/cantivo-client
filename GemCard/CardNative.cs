...
namespace GemCard
{
	...
	public class CardNative : CardBase
	{
		...
		public override string[] ListReaders()
		{
			...
			m_nLastError = NativeMethods.SCardListReaders(m_hContext, null, szListReaders, out pchReaders);
			if (m_nLastError == 0)
			{
		...
		public void EstablishContext(SCOPE Scope)
		{
            ...
            m_nLastError = NativeMethods.SCardEstablishContext((uint)Scope, IntPtr.Zero, IntPtr.Zero, hContext);
			if (m_nLastError != 0)
			{
				string msg = "SCardEstablishContext error: " + (ErrorCodes)m_nLastError;
		...
		public void ReleaseContext()
		{
			if (m_hContext != 0)
			{
                m_nLastError = NativeMethods.SCardReleaseContext(m_hContext);
		...
		public override void Disconnect(DISCONNECT Disposition)
		{
			if (m_hCard != 0)
			{
                m_nLastError = NativeMethods.SCardDisconnect(m_hCard, (uint)Disposition);
				m_hCard = 0;
		...
		public override APDUResponse Transmit(APDUCommand ApduCmd)
		{
		    ...
		    var lengthExpected = ApduCmd.Le;
            var apduData = TransmitRaw(ApduBuffer, lengthExpected);
        ...
        public byte[] TransmitRaw(byte[] ApduBuffer, byte lengthExpected)
        {
            var ioRequest =
                new SCard_IO_Request
                {
                    m_dwProtocol = m_nProtocol,
                    m_cbPciLength = 8
                };
            return TransmitRaw(ApduBuffer, lengthExpected, ioRequest);
        }

	    private byte[] TransmitRaw(byte[] ApduBuffer, byte lengthExpected, SCard_IO_Request ioRequest)
	    {
	        uint RecvLength = (uint)(lengthExpected + APDUResponse.SW_LENGTH);
	        byte[] ApduResponse = new byte[lengthExpected + APDUResponse.SW_LENGTH];

            //Debug send
            Console.Write("Sending: ");
            foreach (var b in ApduBuffer)
            {
                Console.Write(String.Format("{0:X},", b));
            }
            Console.WriteLine();
            
            m_nLastError = NativeMethods.SCardTransmit(
	            m_hCard,
	            ref ioRequest,
	            ApduBuffer,
        ...
        public override void BeginTransaction()
        {
            if (m_hCard != 0)
            {
                m_nLastError = NativeMethods.SCardBeginTransaction(m_hCard);
                if (m_nLastError != 0)
                {
        ...
        public override void EndTransaction(DISCONNECT Disposition)
        {
            if (m_hCard != 0)
            {
                m_nLastError = NativeMethods.SCardEndTransaction(m_hCard, (UInt32)Disposition);
                if (m_nLastError != 0)
                {
        ...
        public override byte[] GetAttribute(UInt32 AttribId)
        {
            ...
            m_nLastError = NativeMethods.SCardGetAttrib(m_hCard, AttribId, attr, out attrLen);
            if (m_nLastError == 0)
            {
                if (attrLen != 0)
                {
        ...
        protected override void RunCardDetection(object Reader)
        {
            ...
            if (NativeMethods.SCardEstablishContext((uint)SCOPE.User, IntPtr.Zero, IntPtr.Zero, phContext) == 0)
            {
                ...
                do
                {
                    if (NativeMethods.SCardGetStatusChange(hContext, WAIT_TIME
